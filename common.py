...
class Fact:
    ...
    def __repr__(self):
        return f'{self.probability} ( {self.predicate} {", ".join(self.arguments)} )'
    ...
    def nl(self, standalone=True):
        ...
        if len(self.arguments) == 1:
            ...
            if self.probability != 1:
                fact_nl = f"With {self.probability} chance, {arg} is {self.predicate}"
            else:
...
class Rule:
    ...
    def nl(self):
        ...
        if self.probability != float(0):
            nl = f"If {lhs_nl} then with {self.probability} chance {rhs_nl}."
        else:
